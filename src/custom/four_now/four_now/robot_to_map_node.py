...
class TransformServiceNode(Node):
    def __init__(self):
        ...
        self.subscription = self.create_subscription(
            ObjectDetectionInfo,
            '/write_to_file',
            self.write_to_file_callback,
            10)
        
        self.collected_obs = pd.DataFrame(columns=["class", 'x', 'y', 'z'])
        self.get_logger().info('transform_point_robot2map service ready.')

    def get_transformed_point(self, point: Point, original_frame: str, destination_frame: str) -> Optional[PointStamped]:
        point_in = PointStamped()
        point_in.header.frame_id = original_frame  # TODO: Update to match your actual robot frame
    ...
    def add_point_to_df(self, point_to_add: PointStamped, class_name: str):
        ...
        self.collected_obs = pd.concat([self.collected_obs, pd.DataFrame([new_point])], ignore_index=True)


    def listener_callback(self, msg: ObjectDetectionInfo):
        if msg.confidence < THRESHOLD_FOR_PROBABILITY_TO_BE_DETECTION:
            return 
        #TODO: add correct frame
        point_in_map_frame = self.get_transformed_point(msg.position, 'base_link', 'map')
