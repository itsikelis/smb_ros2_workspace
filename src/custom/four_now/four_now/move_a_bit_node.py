...
WINDOW_SIZE = 20  # number of Odometry samples kept (~0.5 s)
...
class ConstantTwistPublisher(Node):
    ...
    def __init__(self):
        ...
        self.cmd_pub = self.create_publisher(TwistStamped, "/cmd_vel", 100)
        self.cmd_sub = self.create_subscription(
            TwistStamped, "/cmd_vel", self.se_callback, 10
        )
    ...
    # Subscriber callback
    # ------------------------------------------------------------------
    def se_callback(self, msg: TwistStamped):
        ...
        s = sum(self.pos_history)
        self.is_stopped = abs(s) < 0.001
